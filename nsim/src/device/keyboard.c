...
#define NSIM_KEY_NAME(k) NSIM_KEY_ ## k,

enum {
  NSIM_KEY_NONE = 0,
  MAP(NSIM_KEYS, NSIM_KEY_NAME)
};

#define SDL_KEYMAP(k) keymap[SDL_SCANCODE_ ## k] = NSIM_KEY_ ## k;
static uint32_t keymap[256] = {};

static void init_keymap() {
  MAP(NSIM_KEYS, SDL_KEYMAP)
}
...
void send_key(uint8_t scancode, bool is_keydown) {
  if (nsim_state.state == NSIM_RUNNING && keymap[scancode] != NSIM_KEY_NONE) {
    uint32_t am_scancode = keymap[scancode] | (is_keydown ? KEYDOWN_MASK : 0);
    key_enqueue(am_scancode);
  }
}
#else // !CONFIG_TARGET_AM
#define NSIM_KEY_NONE 0
...
void init_i8042() {
  ...
  i8042_data_port_base[0] = NSIM_KEY_NONE;
