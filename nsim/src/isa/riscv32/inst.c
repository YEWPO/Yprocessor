...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = (int32_t)(src1 + imm));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(rd) = (int32_t)src1 << BITS(imm, 4, 0));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = (int32_t)((uint32_t)src1 >> BITS(imm, 4, 0)));
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = (int32_t)src1 >> BITS(imm, 4, 0));

  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(rd) = (int32_t)src1 + (int32_t)src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw   , R, R(rd) = (int32_t)src1 - (int32_t)src2);
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(rd) = (int32_t)src1 << BITS(src2, 4, 0));
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(rd) = (int32_t)((uint32_t)src1 >> BITS(src2, 4, 0)));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = (int32_t)src1 >> BITS(src2, 4, 0));

  // control transfer instructions
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc, s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc, s->dnpc = ((src1 + imm) & ~0x01));
...
#include "mdu.h"
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(rd) = divw(src1, src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw  , R, R(rd) = divuw(src1, src2));
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(rd) = remw(src1, src2));
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , R, R(rd) = remuw(src1, src2));

  // Zicsr instructions
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, if (BITS(s->isa.inst.val, 11, 7) != 0) R(rd) = CSR(imm); CSR(imm) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = CSR(imm); if (BITS(s->isa.inst.val, 19, 15) != 0) CSR(imm) |= src1);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, R(rd) = CSR(imm); if (BITS(s->isa.inst.val, 19, 15) != 0) csr(imm) &= ~src1);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, if (BITS(s->isa.inst.val, 11, 7) != 0) R(rd) = CSR(imm); CSR(imm) = BITS(s->isa.inst.val, 19, 15));
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, R(rd) = CSR(imm); if (BITS(s->isa.inst.val, 19, 15) != 0) CSR(imm) |= BITS(s->isa.inst.val, 19, 15));
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, R(rd) = CSR(imm); if (BITS(s->isa.inst.val, 19, 15) != 0) CSR(imm) &= ~BITS(s->isa.inst.val, 19, 15));

  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(0xb, s->pc));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NSIMTRAP(s->pc, R(10))); // R(10) is $a0
