...
# compile and run x86-nsim
  "ls /root/nsim"
  "cp -r /root/nsim $TMP_DIR"
  "export NSIM_HOME=$TMP_DIR/nsim"
  "make -C $TMP_DIR/nsim ISA=x86"
  "ls -lh /root/nsim-prog"
  "file /root/nsim-prog/amtest-x86-nsim.elf"
  "$TMP_DIR/nsim/build/x86-nsim --batch --mainargs=h /root/nsim-prog/amtest-x86-nsim.bin"
  "file /root/nsim-prog/microbench-x86-nsim.elf"
  "$TMP_DIR/nsim/build/x86-nsim --batch --mainargs=test /root/nsim-prog/microbench-x86-nsim.bin"

# compile and run riscv64-nsim
  "make -C $TMP_DIR/nsim clean"
  "make -C $TMP_DIR/nsim ISA=riscv64"
  "$TMP_DIR/nsim/build/riscv64-nsim --batch /root/nsim-prog/linux-hello-riscv64-nsim.bin"
)
...
/root/nsimtrap/good-trap
```

* 若在不方便输入的环境(如NSIM, verilator仿真等)中测试, 可采用如下两种方式的其中一种, 避免登录时输入
  * 通过紧急模式登录
```
...
* 修改`nsim/src/device/sdcard.c`中`init_sdcard()`中打开的镜像文件路径, 即可使用制作的镜像.
在i9-9900k上测试, 约90s后看到debian的登录提示符.

* 当以可写方式启动镜像时, NSIM遇到错误或通过Ctrl+C直接退出NSIM时, 可能会损坏镜像的崩溃一致性, 此时可以通过fsck命令修复分区.
