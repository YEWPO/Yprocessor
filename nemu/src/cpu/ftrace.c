...
void ftrace_print() {
  ...
  log_write("frame trace info:\n");
  printf("frame trace info:\n");

  while (ptr != NULL) {
    log_write(FMT_WORD ":", ptr->pc);
    ...
    log_write("[" ANSI_FMT("%s", ANSI_FG_GREEN) "@" ANSI_FMT(FMT_WORD, ANSI_FG_RED) "]\n", strtab + ptr->name, ptr->dnpc);
